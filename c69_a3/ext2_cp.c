...
int main(int argc, char **argv) {
    ...
    */
    ...
    //Constructs proper string representing the native OS path -ADDED
    int nativePathArg = 2;
    int nativeLen = strlen(argv[nativePathArg]); //length of path
    char nativeArgPath[nativeLen+1];
    memcpy(nativeArgPath, argv[nativePathArg],nativeLen ); // Compies variables
    nativeArgPath[nativeLen] = '\0'; // List

    //Constructs proper string representing the path -ADDED
    int pathArg = 3;
    int len = strlen(argv[pathArg]); //length of path
    char argPath[len+1];
    memcpy(argPath, argv[pathArg],len ); // Compies variables
    argPath[len] = '\0'; // List
    ...
    while (current_size < EXT2_BLOCK_SIZE){
    	...
    	if (current_size + node_dir->rec_len == EXT2_BLOCK_SIZE){
		    ...
		    par_dir->file_type = EXT2_FT_REG_FILE;

		    // get the new name
		    char dup_path[nativeLen];
		    strcpy(dup_path, nativeArgPath);
		    char *ret;
