...
int get_inode_index(char fullPath[],struct ext2_inode* inode_table){
  ...
  memcpy(reducedPath, fullPath,strlen(fullPath));
  if (strcmp(fullPath, "/") == 0){
    return inodeTracker ;
  }
  
  while(inodePosition > 1){
   // position != size, we add one
   ...
   memcpy(reducedPath,(char*)(reducedPath + inodePosition ),strlen((char*)(reducedPath + inodePosition )));
   reducedPath[strlen((char*)(reducedPath + inodePosition ))] = '\0';
   bool found= false;
   int blockIterator;
   int totalSize =0;
