 ...
 */
	...
	syscall = table[reg.ax].f(reg); //is this the return value? yes.

	spin_unlock(&pidlist_lock);
	spin_unlock(&calltable_lock);

	return syscall; // Just a placeholder, so it compiles with no warnings!
...
int is_intercepted(int syscall){
	return table[syscall].intercepted == 1;
}

int is_invalid(int pid){
	return pid < 0 || (pid != 0 && pid_task(find_vpid(pid), PIDTYPE_PID) == NULL);
}

int caller_pid(int pid){
	return  check_pid_from_list(pid, current->pid) != -EPERM;
}
 ...
 *      - REQUEST_SYSCALL_INTERCEPT 1 to intercept the 'syscall' argument
 *      - REQUEST_SYSCALL_RELEASE 2 to de-intercept the 'syscall' argument
 *      - REQUEST_START_MONITORING 3 to start monitoring for 'pid' whenever it issues 'syscall'
 *      - REQUEST_STOP_MONITORING 4 to stop monitoring for 'pid'
 ...
 */
 ...
 if((syscall > MY_CUSTOM_SYSCALL) && (syscall <= NR_syscalls)){ //validity of syscall
	 if((cmd == REQUEST_SYSCALL_INTERCEPT) || (cmd == REQUEST_SYSCALL_RELEASE)){ // intercept / de-intercept system call
		 if(current_uid() == 0){ // root
				 if(cmd == REQUEST_SYSCALL_RELEASE){ // de-intercept
					 if(table[syscall].intercepted == 1){ // is intercepted

						 spin_lock(&calltable_lock);
						 spin_lock(&pidlist_lock);
						 set_addr_rw((unsigned long) sys_call_table);
						 sys_call_table[syscall] = table[syscall].f; //deintercept sys_call_table
					 ...
					 if(table[syscall].intercepted == 1){
						 ...
						 spin_lock(&pidlist_lock);
						 set_addr_rw((unsigned long) sys_call_table);
						 table[syscall].f = sys_call_table[syscall]; //store system calls 
						 sys_call_table[syscall] = interceptor; //intercept sys_call_table
					  	 set_addr_ro((unsigned long) sys_call_table);
	 ...
	 if (cmd == REQUEST_START_MONITORING)
     	{
     		...
     		if (pid == 0) {
     			...
     			else if (table[syscall].monitored == 2 && table[syscall].listcount == 0)
     			            spin_unlock(&calltable_lock);
                     		spin_unlock(&pidlist_lock);
     				return -EBUSY;
     			...
     			if (table[syscall].monitored == 1)
     				return_val = check_pid_monitored(syscall, pid) ? -EBUSY : add_pid_sysc(pid, syscall);
     			else if (table[syscall].monitored == 2)
     				return_val = check_pid_monitored(syscall, pid) ? add_pid_sysc(pid, syscall) : -EBUSY;
     			else // Monitor a specifid PID.
     			{
     				return_val = add_pid_sysc(pid, syscall);
     	...
     	else if (cmd == REQUEST_STOP_MONITORING)
        	{
        		...
        		if (!current_uid() == 0 && (pid == 0 || !caller_pid(pid)))
        		{
        			spin_unlock(&pidlist_lock);
        			return -EPERM;
        		}
        		...
        		} else { // remove a specific PID
        			if (table[syscall].monitored == 1)
        				return_val = check_pid_monitored(syscall, pid) ? del_pid_sysc(pid, syscall) : -EINVAL;
        			else if (table[syscall].monitored == 2)
        				return_val = check_pid_monitored(syscall, pid) ? -EINVAL : add_pid_sysc(pid, syscall);
        		}
       ...
       }else{	// send error here, syscall is not valid
...
//for loop to inialize all the entries in table goes here
spin_lock(&calltable_lock);
spin_lock(&pidlist_lock);
int i;
...
// struct list_head lst1;
// INIT_LIST_HEAD(&lst1);
// table[MY_CUSTOM_SYSCALL] = (mytable){sys_call_table[MY_CUSTOM_SYSCALL], 0, 0, 0, lst1};

 printk(KERN_DEBUG "ASS1 %d", table[MY_CUSTOM_SYSCALL].intercepted);

//table[MY_CUSTOM_SYSCALL] = (mytable) { .pt_regs = sys_call_table[MY_CUSTOM_SYSCALL], .intercepted = 0, .monitored = 0, .listcount = 0, .list_h = INIT_LIST_HEAD (&my_list)};
 ...
 */
	...
	//Restore all intercepted system calls
