...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
		...
		if((coremap[frame].pte)->frame & PG_DIRTY) {
			int swap_off = (coremap[frame].pte)->swap_off;
			int offset = swap_pageout(frame, swap_off);
...
char *find_physpage(addr_t vaddr, char type) {
	...
	if(!(p->frame & PG_VALID)) {
    	//Occurs when invalid
    		...
    		//Occurs when on swap
    			...
    			int offset = p->swap_off;

    			if(swap_pagein(p->frame, offset) != 0) {
    				perror("Swap failure.");
    				exit(1);
    		...
    		//Occurs when not on swap
    			p->frame = allocate_frame(p);
    			init_frame(p->frame, vaddr);
    			p->frame = p->frame << PAGE_SHIFT;
    			p->frame = p->frame | PG_DIRTY;

    			//cold miss, but still a miss
	...
	//sets valid bit as below can only change valid status bit
	p->frame = p->frame | PG_VALID;

	//sets reference bit as below can only change referenced status bit
	p->frame = p->frame | PG_REF;

	//check if page will be written to and mark accordingly
	if(type == 'M' || type == 'S') {
		//sets dirty bit as below can only change dirty status bit
		p->frame = p->frame | PG_DIRTY;
	}
